...
namespace IngameScript.TargetTracking
{
    ...
    public class LidarTracker
    {
        ...
        public void PingPlannedTargets()
        {
            ...
            while (pingqueue.Count > 0 && Now() > pingqueue.peekFirst())
            {
                ...
                if (target.Search < 100)
                {
                    ...
                    ValueTuple<bool, bool> result = PingEntity(target);
                    if (result.Item1)
                    {
                        if (result.Item2)
                        {
        ...
        ValueTuple<bool, bool> PingEntity(TrackedEntity e)
        {
            Vector3D targetPosition = e.SearchPosition(Now(), MyGrid.GetPosition());
            ValueTuple<bool, TrackedEntity> result = PingPosition(targetPosition);
            ...
            return new ValueTuple<bool, bool>(result.Item1, hitIntendedTarget);
        }

        // returns pair (can_ping_target_position, pinged_target)                 
        public ValueTuple<bool, TrackedEntity> PingPosition(Vector3D targetPosition)
        {
            ...
            if (lidarsearchresult.HasValue)
            {
                ...
                if (!obs.IsEmpty())
                {
                    ...
                    ValueTuple<bool, TrackedEntity> rtn = new ValueTuple<bool, TrackedEntity>(true, Entities.Update(ref obs));
                    if (rtn.Item2 != null)
                    {
            ...
            return new ValueTuple<bool, TrackedEntity>(lidarsearchresult.HasValue, null);
        }
